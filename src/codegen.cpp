    ...
    */
    ...
    for (i = 0; i < nreq; i++) {
        ...
        if (s == unused_sym || vi.value.constant) {
            ...
            // the world age.
            if (i == 0 && ctx.is_opaque_closure) {
                jl_cgval_t closure_world = emit_getfield_knownidx(ctx, theArg, 2, (jl_datatype_t*)argType, jl_memory_order_notatomic);
                ...
                theArg = convert_julia_type(ctx,
                    emit_getfield_knownidx(ctx, theArg, 0, (jl_datatype_t*)argType, jl_memory_order_notatomic),
                    vi.value.typ);
            }
